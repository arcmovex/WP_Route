 ...
 *
 * @author     Anthony Budd
 * @link https://github.com/anthonybudd/WP_route Github Repository
 */
final class WP_Route {
    ...
    private function exceptUnlistedPages() {
        ...
        return in_array($requestURI, $routes);
    }
    ...
    public function handle() {
        if ($this->exceptUnlistedPages()) {
            ...
            foreach ($routes as $key => $route) {
                ...
                if ($route->route == implode('/', $tokenizedRequestURI)) {
                    $matchedRoute = $key;
                }
            }

            $routes = array_values($routes);
            if (isset($routes[$matchedRoute])) {
                ...
                } elseif (isset($routes[$matchedRoute]->redirect)) {

                    $redirect = $routes[$matchedRoute]->redirect;
                    header("Location: {$redirect}", TRUE, $routes[$matchedRoute]->code);
                    die;
                } else {
